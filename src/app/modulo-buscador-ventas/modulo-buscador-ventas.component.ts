...
export class ModuloBuscadorVentasComponent implements OnInit {
  ...
  itemSelectedDelete: VentaNodos = null
  ...
  ngOnInit() {
    ...
    this.loadDataComponent().subscribe(
      (resp) => {
        ...
        this.filterDesde = moment().clone().startOf('month').format('YYYY-MM-Do')

        this.filterHasta = moment().format('YYYY-MM-Do')
  ...
  listarPorEstadoVenta() {
    ...
    this.mbvService
      .getVentaByProyectoAndEstado(this.idProyecto, parseInt(this.filterIdEstadoVenta))
      .subscribe((resp) => {
        this.itemsLista = new MatTableDataSource<VentaNodos>(resp)
      })
  }
  ...
  selectToDelete(itemselected: VentaNodos) {
    this.itemSelectedDelete = new VentaNodos()
    this.itemSelectedDelete = { ...itemselected }
  }

  btnCerrarModalDelete() {
    document.getElementById('cerrarModalEliminarVenta').click()
  }

  eliminarVentaSeleccionada() {
    this.mbvService.deleteVentaById(this.itemSelectedDelete.idVenta).subscribe(
      (resp) => {
        this.openSnackBar('success', '✓ Venta eliminada con éxito', 'Cerrar')
        this.btnCerrarModalDelete()
        this.aplicarFiltros()
      },
      (error) => {
        console.error(error)
      ...
      .subscribe(
        (data) => {
          ...
          data.forEach((element) => {
            ...
            tmpItem['fechaNacimiento'] = this.datepipe.transform(
            ...
            for (let x = 0; x < maximoCantidadInmuebles; x++) {
              ...
              if (element.listVentaInmueble[x] != undefined) {
                tmpItem['tipoInmueble' + numitem] =
                  element.listVentaInmueble[x].inmueble.tipoInmueble.nombre
            ...
            tmpItem['fechaSeparacion'] = this.datepipe.transform(
              element.venta.fechaSeparacion,
              'dd/MM/yyyy'
            )
            tmpItem['fechaMinuta'] = this.datepipe.transform(
              element.venta.fechaMinuta,
              'dd/MM/yyyy'
            )
            tmpItem['fechaDesembolso'] = this.datepipe.transform(
              element.venta.fechaDesembolso,
              'dd/MM/yyyy'
            )
            ...
            for (let x = 0; x < maximoCantidadPagos; x++) {
              ...
              if (element.listPagos[x] != undefined) {
                ...
                totalPorcentajePagos =
                  (totalPagos / element.venta.financiamiento.montoFinanciado) * 100
                ...
                tmpItem['fechaPago' + numitem] = this.datepipe.transform(
                  element.listPagos[x].fecha,
                  'dd/MM/yyyy'
                )
