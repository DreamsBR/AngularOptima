...
export class ConsultaVentasDetalleComponent implements OnInit {
  ...
  filtrar() {
    this.ventaService.getVentasPorDni(this.clienteSeleccionado.idCliente, 1, 1).subscribe((jsonVentas) => {
  ...
  obtenerVentasProyecto() {
    this.ventaService
      .getVentasByProyectoEstadoFeciniFecfin(this.paramIdProyecto, this.estadoventaSeleccionado, this.fechaDesde, this.fechaHasta)
  ...
  obtenerSoloFecha() {
    this.ventaService.getVentasProyectoPorFecha(this.paramIdProyecto, this.fechaDesde, this.fechaHasta).subscribe((ventasJsonRe) => {
